...
int main(int argc, char *argv[])
{
	...
	struct process_struct tempRunning[100];
	int tempRunningLength = 0;
	...
	while (endFlag != 0 || howManyAreIn != n)
	{
		...
		if (bqf != 0)
		{
			...
			for (int j = bqe; j < bqf; j++)
			{
				...
				else
				{
					...
					bqe++;
					// rqf++;
					// if (bq[j].pid != current_proccess.pid)
					// {
						tempRunning[tempRunningLength] = bq[j];
						tempRunningLength++;
					// }
				}
			}
		...
		if (i == rft && rft != 0)
		{
			...
			else if (current_proccess.pt > 0)
			{
				...
				// rqf++;
				tempRunning[tempRunningLength] = current_proccess;
				tempRunningLength++;
				old_proccess.pid = current_proccess.pid;
			}
			else
		...
		for (int k = 0; k < n; k++)
		{
			...
			if (ps[k].at == i && ps[k].pt != 0)
			{
				...
				// rq[rqf] = ps[k];
				// rqf++;
				if (ps[k].pid != current_proccess.pid)
				{
					tempRunning[tempRunningLength] = ps[k];
					tempRunningLength++;
				}
			}
		}
		...
		// Sort Arrivals
		qsort((void *)tempRunning, tempRunningLength, sizeof(struct process_struct), comparatorPID);
		for (int k = 0; k < tempRunningLength; k++)
		{
			rq[rqf] = tempRunning[k];
			rqf++;
		}
